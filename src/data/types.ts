...
type Base = {
  ...
  level?: 1 | 2 | 3;
  /** Подробные навыки для диагностики (одно задание — несколько навыков). */
  skills?: string[];
  /** Схема временной линии. Не содержит названия правильного времени. */
  timeline?: TimelineSpec;
};

export type TimelineSpec = {
  /** Начало процесса (слева), если оно известно. */
  from?: string;
  /** Точка в будущем, о которой спрашивают. */
  point: string;
  /** Подпись действия над полосой. */
  action: string;
  /**
   * through — процесс идёт через точку (виден В момент);
   * done — действие закончено до точки;
   * until — процесс тянется от начала до точки.
   */
  shape: "through" | "done" | "until";
};
