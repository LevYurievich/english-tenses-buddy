/** Диагностика модуля «Все Past»: по временам, навыкам и категориям ошибок. */
import type { Exercise } from "@/data/types";
import { CATEGORY_TITLES } from "@/data/error-categories";
import { SKILL_TITLES } from "@/data/skills";
import { ALL_PAST_ID, ALL_PAST_TRAINING, PAST_FINAL_BANK, PAST_LEVELS } from "@/data/all-past";
import { getTenseProgress, type ProgressState } from "./progress";
import type { Stat } from "./all-present-stats";

export type { Stat };

export const PAST_TENSE_TITLES: Record<string, string> = {
...
const BANK: Exercise[] = [...ALL_PAST_TRAINING, ...PAST_FINAL_BANK];
...
export function pastModuleResults(state: ProgressState | null): Record<string, boolean> {
  if (!state) return {};
  return getTenseProgress(state, ALL_PAST_ID).results ?? {};
}

export function pastByTense(results: Record<string, boolean>): Stat[] {
  const stats = group(
    results,
    (ex) => ex.targetTense,
    (k) => PAST_TENSE_TITLES[k] ?? k,
  );
  const order = Object.keys(PAST_TENSE_TITLES);
  return stats.sort((a, b) => order.indexOf(a.key) - order.indexOf(b.key));
}

export function pastBySkill(results: Record<string, boolean>): Stat[] {
  return group(
    results,
    (ex) => ex.skill,
    (k) => SKILL_TITLES[k] ?? k,
...
export function pastByCategory(results: Record<string, boolean>): Stat[] {
  return group(
    results,
    (ex) => ex.errorCategory,
...
export function pastLevelProgress(state: ProgressState | null) {
  const done = state ? getTenseProgress(state, ALL_PAST_ID).doneExercises : [];
  return PAST_LEVELS.map((level) => ({
    id: level.id,
    title: level.title,
...
export function pastLevelsCompleted(state: ProgressState | null): boolean {
  return pastLevelProgress(state).every((l) => l.done >= l.total);
}

/** 1–3 самых слабых навыка — для блока «Твои слабые места». */
export function pastWeakSpots(results: Record<string, boolean>): Stat[] {
  return [...pastByCategory(results), ...pastByTense(results)]
...
export function pastRecommendations(results: Record<string, boolean>): string[] {
  const tenses = pastByTense(results);
  const strong = tenses.filter((s) => s.total >= 2 && s.correct / s.total >= 0.85);
  const weak = tenses
    .filter((s) => s.total >= 2 && s.correct / s.total < 0.8)
    .sort((a, b) => a.correct / a.total - b.correct / b.total);
  const weakCats = pastByCategory(results)
...
export function pastWeakAreaExercises(results: Record<string, boolean>, limit = 12): Exercise[] {
  const catScore = new Map(
    pastByCategory(results).map((s) => [s.key, s.total ? s.correct / s.total : 1]),
  );
  const tenseScore = new Map(
    pastByTense(results).map((s) => [s.key, s.total ? s.correct / s.total : 1]),
  );

  const scored = ALL_PAST_TRAINING.map((ex) => {
