...
function ComparePage() {
  ...
  const percentFuturePerfect = percentFor(progress, "compare-future-perfect");
  const percentFpcFuture = percentFor(progress, "compare-future-perfect-continuous");
  ...
  return (
    <div className="space-y-6">
      ...
      <article className="card-surface flex flex-col gap-3 p-5">
        <div className="flex items-start justify-between gap-2">
          <h2 className="text-lg">Future Simple vs Future Perfect</h2>
          <span className="rounded-full bg-success/15 px-2.5 py-1 text-[11px] font-bold text-success">
            Доступно
          </span>
        </div>
        <p className="text-sm text-muted-foreground">
          ЧТО ПРОИЗОЙДЁТ? → WILL + V1 • ЧТО УЖЕ БУДЕТ ГОТОВО К МОМЕНТУ? → WILL HAVE + V3
        </p>
        <ProgressBar value={percentFuturePerfect} label="Прогресс" />
        <Link
          to="/compare/future-perfect"
          className="mt-1 inline-flex items-center justify-center rounded-xl bg-primary px-4 py-2 text-sm font-bold text-primary-foreground transition hover:brightness-110"
        >
          {percentFuturePerfect > 0 ? "Продолжить" : "Начать"}
        </Link>
      </article>
      <article className="card-surface flex flex-col gap-3 p-5">
        <div className="flex items-start justify-between gap-2">
          <h2 className="text-lg">Future Perfect vs Future Perfect Continuous</h2>
          <span className="rounded-full bg-success/15 px-2.5 py-1 text-[11px] font-bold text-success">
            Доступно
          </span>
        </div>
        <p className="text-sm text-muted-foreground">
          ЧТО БУДЕТ ГОТОВО? → WILL HAVE + V3 • КАК ДОЛГО? → WILL HAVE BEEN + V-ING
        </p>
        <ProgressBar value={percentFpcFuture} label="Прогресс" />
        <Link
          to="/compare/future-perfect-continuous"
          className="mt-1 inline-flex items-center justify-center rounded-xl bg-primary px-4 py-2 text-sm font-bold text-primary-foreground transition hover:brightness-110"
        >
          {percentFpcFuture > 0 ? "Продолжить" : "Начать"}
        </Link>
      </article>
